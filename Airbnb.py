...
def datafr():
    df= pd.read_csv("C:\\Users\HP\\updated_airbnb.csv")
    return df
...
if select == "Home":
    ...
    image1= Image.open("C:\\Users\\HP\\Airbnb\\image1.png")
    st.image(image1)
...
if select == "Data Exploration":
    ...
    with tab1:
        ...
        with col1:
            ...
            df_bar= pd.DataFrame(df2.groupby("property_type")[["price","review_scores.review_scores_accuracy","number_of_reviews"]].sum())
            df_bar.reset_index(inplace= True)

            fig_bar= px.bar(df_bar, x='property_type', y= "price", title= "PRICE FOR PROPERTY_TYPES",hover_data=["number_of_reviews","review_scores.review_scores_accuracy"],color_discrete_sequence=px.colors.sequential.Redor_r, width=600, height=500)
        ...
        with col1:
            ...
            hostresponsetime= st.selectbox("Select the host.host_response_time",df4["host.host_response_time"].unique())
    ...
    with tab2:
        ...
        def datafr():
            df_a= pd.read_csv("C:\\Users\HP\\updated_airbnb.csv")
            return df_a
        ...
        with col1:
            ...
            df_a_sunb_30= px.sunburst(df2_a, path=["room_type","bed_type","address.location.is_location_exact"], values="availability.availability_30",width=600,height=500,title="Availability_30",color_discrete_sequence=px.colors.sequential.Peach_r)
        ...
        with col2:
            ...
            df_a_sunb_60= px.sunburst(df2_a, path=["room_type","bed_type","address.location.is_location_exact"], values="availability.availability_60",width=600,height=500,title="Availability_60",color_discrete_sequence=px.colors.sequential.Blues_r)
        ...
        with col1:
            ...
            df_a_sunb_90= px.sunburst(df2_a, path=["room_type","bed_type","address.location.is_location_exact"], values="availability.availability_90",width=600,height=500,title="Availability_90",color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
            st.plotly_chart(df_a_sunb_90)

        with col2:

            df_a_sunb_365= px.sunburst(df2_a, path=["room_type","bed_type","address.location.is_location_exact"], values="availability.availability_365",width=600,height=500,title="Availability_365",color_discrete_sequence=px.colors.sequential.Greens_r)
        ...
        fig_df_mul_bar_a = px.bar(df_mul_bar_a, x='host.host_response_time', y=['availability_30', 'availability_60', 'availability_90', "availability_365"], 
    ...
    with tab4:
        ...
        fig_4 = px.scatter_mapbox(df, lat='address.location.coordinates[0]', lon='address.location.coordinates[1]', color='price', size='accommodates',
